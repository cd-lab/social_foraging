...
# create stimuli
	...
	canPicture = visual.ImageStim(win, image="can.png", units='height', pos= (horCenter, verCenter),
		size=0.15, ori=0.0)
	bottlePicture = visual.ImageStim(win, image="bottle.png", units='height', pos= (horCenter, verCenter),\
		size=0.13, ori=0.0)
	...
	avatar = visual.ImageStim(win, image="avatar.png", units='height', size = 0.2, ori=0.0, pos = (horCenter + 0.3, verCenter))
	you = visual.ImageStim(win, image="you.png", units='height', size = 0.2, ori=0.0, pos = (horCenter - 0.3, verCenter))
	# return outputs
	...
	'canPicture' : canPicture, 'bottlePicture' : bottlePicture, "avatar": avatar, "you" : you}
	return(outputs)
...
def showTrial(win, expParas, expInfo, expHandler, stims, rwdSeq_, htSeq_, ifPrac):
	...
	for blockIdx in range(nBlock):
		...
		trialEarnText = visual.TextStim(win=win, ori=0, font=u'Arial', bold = True, units='height',\
			pos=[horCenter, verCenter + 0.2], height=0.1, color=[-1, -1, -1], colorSpace='rgb') 		
...
def showTrialSocial(win, expParas, expInfo, expHandler, stims, rwdSeq_, htSeq_, ifPrac):
	...
	nFbFrame = math.ceil((expParas['travelSec'] - expParas['decsSec']) / expInfo['frameDur'])
	nDecsFrame = math.ceil(expParas['decsSec'] / expInfo['frameDur'])
	frameIdxOtherStart = round(expParas['fbSelfSec'] / expInfo['frameDur'])
	...
	for blockIdx in range(nBlock):
        ...
        # create feedback 
		trialEarnText = visual.TextStim(win=win, ori=0, font=u'Arial', bold = True, units='height',\
		pos=[horCenter - 0.3, verCenter + 0.2], height=0.1, color=[-1, -1, -1], colorSpace='rgb') 	
		...
		while (not ifPrac and realLeftTime > 0) or (ifPrac and trialIdx < len(expParas['unqHts'])) :
			...
			while frameIdx < nFbFrame and realLeftTime > 0:	
				...
				if trialEarnings == expParas['rwdHigh']:
					...
					canPicture.draw()
				if frameIdx > frameIdxOtherStart :
